...
#     #------------------------------------------------
#     #Create a tash to fluctuate CPU and memory usage
#     #------------------------------------------------
...
def create_ecs_cluster(cluster_name='my-ecs-cluster', task_definition_name='my-highload-task'):
    try:
        # Step 1: Create IAM Role for Fargate Task Execution
        role_name = 'ecsTaskExecutionRole'
        assume_role_policy = {
        ...
        # Step 2: Register Task Definition for Fargate with High CPU and Memory Load
        task_definition = {
            "family": task_definition_name,
            "networkMode": "awsvpc",  # Required for Fargate
            "requiresCompatibilities": ["FARGATE"],
            ...
            "containerDefinitions": [
                {
                    ...
                    "command": [
                        "/bin/sh", "-c",
                        # Shell script to maintain continuous high CPU and memory usage
                        """
                        ...
                        # Allocate as much memory as possible without causing OOM kill
                        for i in $(seq 1 1000); do
                            # Each iteration creates ~4MB file
                            dd if=/dev/urandom of=/tmp/mem_file_$i bs=4M count=1 2>/dev/null || break
                        done
                        
                        # Keep the container running
                        while true; do
                            sleep 10
                            # Ensure CPU remains loaded
                            for i in $(seq 1 $(nproc)); do
                                if ! ps aux | grep -v grep | grep -q "dd if=/dev/zero"; then
                                    dd if=/dev/zero of=/dev/null bs=1M &
                                fi
                            done
                        done
