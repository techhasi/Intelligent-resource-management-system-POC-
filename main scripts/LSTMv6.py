...
class EnhancedCloudEnvironment:
    def __init__(self, data, services=['ec2', 'rds', 'ecs']):
        ...
        self.performance_weight = 8.0  # Increased weight for performance vs cost
            ...
            for i, service in enumerate(self.services):
                if i in actions:
                    ...
                    if utilization < self.cpu_threshold_very_low:
                        ...
                        service_reward = 8.0 * (1.0 - distance**2)  # Increased reward for optimal range
                        ...
                        service_reward = 3.0 + 2.0 * high_factor  # Increased reward range
                        ...
                        service_reward = -4.0 + 1.5 * high_factor  # More severe penalty range
                    else:
                        # Extreme under-provisioning - very severe penalty
                        service_reward = -10.0 - (utilization - self.cpu_threshold_very_high) / 5.0
            ...
            if random.random() > self.epsilon:
                ...
                for i in range(self.num_services):
                    ...
                    for a in range(3):
                        if q_values[a] == 0:  # Not visited before
                            if a == 0:  # Scale down
                                q_values[a] = 0.3  # Lower optimism for scale down
                            elif a == 1:  # No change
                                q_values[a] = 0.6  # Higher optimism for no change
                            else:  # Scale up
                                q_values[a] = 0.5  # Optimism value
                    
                    # HIGH CPU PROTECTION: Prevent scaling down at high CPU
                    if cpu_util > 70:  # Lowered threshold from 80 to 70
                        # Heavily penalize scale down at high CPU
                        q_values[0] -= 15.0  # Increased from 10.0
                        # Boost scale up at high CPU
                        q_values[2] += 5.0  # Increased from 2.0
                        # Also boost no-change at high CPU
                        q_values[1] += 2.0
                ...
                for i in range(self.num_services):
                    ...
                    if cpu_util > 70:
                        ...
                        probs = [0.01, 0.49, 0.50]  # [down, no change, up] - made scale down very unlikely
                    elif util_per_resource > 70:
                        # High utilization - likely need to scale up
                        probs = [0.05, 0.25, 0.70]  # [down, no change, up]
                    elif util_per_resource < 30:
                        # Low utilization - likely need to scale down
                        probs = [0.6, 0.3, 0.1]  # [down, no change, up] - reduced scale down probability
                    else:
